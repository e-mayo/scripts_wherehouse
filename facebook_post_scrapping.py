"""
Created on Sat Mar 13 02:15:35 2021
...
login_btn = driver.find_element_by_xpath('/html/body/div[1]/div[2]/div[1]/div/div/div/div[2]/div/div[1]/form/div[2]/button')
print('loging')
random_sleep()
login_btn.click()

url = 'https://www.facebook.com/CentroInvestigacionesSocialesCuba/posts/1167580170344179'
random_sleep()
driver.get(url)
...
def load_more_comments(max_scroll=15):
    scrolls = 0
    while True:
        try:
            print(f'scroll: {scrolls}')
            load_more_comments_btn = driver.find_element_by_xpath(
                'html/body/div[1]/div/div[1]/div/div[3]/div/div/div[1]/div[1]/div[4]/div[1]/div/div/div/div/div/div/div/div/div/div[1]/div/div[2]/div/div[4]/div/div/div[2]/div[4]/div[1]/div[2]/span/span')
            load_more_comments_btn.click()
            random_sleep()
            loaded_comments, total_comments = get_comments_status()
            if scrolls >= max_scroll:
                break
            if not loaded_comments < total_comments:
                break
        except:
            pass
        finally:
...
def get_interactions_type(comment):
    ...
    random_sleep()
    try:
        interactions = comment.find_element_by_xpath(
            './child::div/div/div/div/div/div/div/div/span')
    except:
        return {}
    hover(interactions)
    interactions_list = driver.find_elements_by_xpath(
        '/html/body/div[1]/div/div[1]/div/div[3]/div/div/div[2]/div/div/div[1]/span/div/div/span/div')
    interaction_dict = dict()
    for interaction in interactions_list:
        svg = get_svg(interaction)
        interaction_total = interaction.text.split('\n')[0]
        interaction_type = likes_type[svg]
        print(f'{interaction_type}: {interaction_total}')
        interaction_dict.setdefault(interaction_type, interaction_total)
    return interaction_dict


def read_replies(comment):
    replies = comment.find_elements_by_xpath("./child::div/div/ul/li")
    return replies


def check_replies(comment):
    text = comment.find_element_by_xpath("./child::div[2]/div").text
    if text == '':
        print('no replies')
        return False
    else:
...
def load_replies(comment):
    random_sleep()
    try:
        load_replies_btn = comment.find_element_by_xpath(
            "./child::div[2]/div/div/div[2]")
        if load_replies_btn.get_attribute('role') == 'button':
            if 'Ocultar' in load_replies_btn.text:
                print('all replies already loaded')
            else:
                load_replies_btn.click()
                print('loaded replies')
            return True
        else:
            return False
    except:
        print('no replies button found or all replies loaded')
        return False


def load_all_replies(comment):
    while True:
        random_sleep()
        show_more_replies_btn = comment.find_elements_by_xpath(
            "./child::div[2]/div/div")[-1]
        if 'Ver ' in show_more_replies_btn.text:
            show_more_replies_btn.click()
        else:
            break
            print('all replies loaded')
    return True
...
def load_full_comment(comment):
    if not 'Ver m' in comment.text:
        print('full comment loaded')
        return True
    random_sleep()
    try:
        c = driver.find_element_by_xpath(
            '/html/body/div[1]/div/div[1]/div/div[3]/div/div/div[1]/div[1]/div[4]/div[1]/div/div/div/div/div/div/div/div/div/div[1]/div/div[2]/div/div[4]/div/div/div[2]/ul/li[6]')
        ver_mas_btn = c.find_element_by_xpath(
            './child::div/div/div/div/div/div/div/div/div/div/span/div/div/div')
        ver_mas_btn.click()
        print('full comment loaded')
        return True
    except:
...
def write_replies(replies):
    with open('replies.txt', 'w', encoding='UTF-8') as f:
        for i, r in enumerate(replies):
            f.writelines(f'reply {i}\n')
            f.writelines(r.text)


def write_json(data, file_name='fb_post_scrapping.json'):
...
load_more_comments(max_scroll=700)
...
for c in comments:
        ...
        if not c:
            continue
        c_info = get_comment_info(c)
        if check_replies(c):
            replies_info = []
            r_status = load_replies(c)
            more_replies = load_all_replies(c)
            replies = read_replies(c)
            for r in replies:
                replies_info.append(get_comment_info(r))
...
def hide_user(raw_text):
    safe_text = raw_text 
    for usr in _USERS_:
        safe_text = safe_text.replace(usr,'')
    return safe_text


for c in commentaries:
    c['comment'] = hide_user(c.get('comment'))
    if not c.get('replies'): continue
    for r in c.get('replies'):
        r['comment'] = hide_user(r.get('comment'))

write_json(commentaries, file_name='fb_post_scrapping_list.json')
data = {}
